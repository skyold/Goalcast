...
### 为什么 OA_HT_V2.py 要用 "12:00 ~ 次日 12:00" 这个 noon-to-noon 划分?

这是**博彩 / 体育媒体行业通用的"比赛日"约定**,跟时区无关,跟用户看球节奏有关:

| 时间(上海时间) | 球场 | 玩家行为 |
|---|---|---|
| **下午 12:00** | — | 中午前 — 自然日切点 |
| 17:00 - 19:00 | 亚洲杯、中超 | 看球阶段 1 |
| 20:00 - 23:00 | 英超 / 五大联赛黄金时段 | 看球阶段 2 |
| 03:00 - 06:00(次日) | 南美 / 后场欧战 | 看球阶段 3,凌晨场 |
| **次日 11:59 之前** | — | 总结昨晚 |

如果按**自然日**(00:00 - 23:59)划"今天",**凌晨 3 点的南美赛事会被丢到第二天**的列表里,但玩家心智里它和昨晚 20:00 的英超是同一个"看球夜"。所以 OA 脚本用 **12:00 作为切口**:今天中午到明天中午之间所有开球的比赛 = "今天的清单"。

365bet 客户端、虎扑赛程页等很多体育产品也用 noon cutoff,作者沿用此惯例。

### 为什么 gs_ht_ev.py 不需要?

CLI 脚本是**手动**操作:用户输入日期 → 脚本拉那一天的比赛 → CLI 打印结果。每次操作都要先选日期,所以**需要"比赛日"概念**。

信号管线是**自动**操作:
- snapshot worker 每 15 分钟扫 fixture 池
- 对**每一场**比赛、**每个 waypoint**(48h/24h/6h/1h/kickoff)自动算 KEN-HT-EV
- 结果落到 `signals_snapshot` 表
- 前端 catalog UI 可按"日期 / 联赛 / strength / scope"任意维度过滤

**信号本身不需要"今天看哪些比赛"的边界,因为信号是给所有比赛都算的**。"今天看哪些"是 UI 层的事,信号层不管。

### 评分 ➖ N/A 的含义

这个维度对信号管线不适用(CLI 工具 → 信号管线的必然换装),**不是 bug,不需要对齐**。CLI 工具一次只看一天所以要切日;信号管线全自动算所以没切日需求。两者都没错,各干各的。

### 一个相关但不同的概念:模拟盘的"12:00 起轮询"

作者更早的反馈里提到:

> "从当天中午 12 点开始这样去做,然后那场球如果一直不达到 1V 投注标准,就读他滚球..."

那是 **模拟盘下单触发器**的需求,**不是信号本身**。意思是"市场赔率轮询的起始时间用 12:00",而不是"信号在哪些比赛上算"。具体设计放在 `docs/PRD/paper-trading-realism.prd.md` 的限价单 MVP 阶段讨论,跟本文件的 gs_ht_ev 信号实现解耦。
