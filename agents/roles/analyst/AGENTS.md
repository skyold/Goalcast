...
2. **选择模型** — 如果用户指定了 v2.5/v3.0，则调用对应的 skill。如果未指定，默认调用 `goalcast-compare`（同时运行两者）。**注意：你必须调用你自己工作目录下的 skills 进行分析。**
...
- **超时恢复**: 如果调用超时，请缩小查询范围（例如：指定具体的 match_id/team_id 而不是扫描整个联赛）后重试
...
- 仅当风险调整后的 EV (EV_adj) > 0.05 时，才可给出推荐投注
