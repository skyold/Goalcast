...
from datasource.datafusion.resolver import ResolvedData, CACHE_TTL, _is_error_response, _find_team
from datasource.datafusion.quality import assess_standings_quality, assess_odds_quality
from datasource.datafusion.models import get_understat_league_code
...
def _extract_team_expected_history(raw: Any, participant_id: Optional[int] = None, location: Optional[str] = None) -> Optional[dict]:
    entries = _extract_expected_entries(raw)
    if participant_id is not None:
        matching_entries = [
            entry for entry in entries
            if str(entry.get("participant_id")) == str(participant_id)
    ...
    return {
        ...
        "location": location,
        "metrics": _aggregate_expected_metrics(entries),
        "entries": entries,
    }


def _extract_fixture_expected(raw_fixture: Any, home_id: int, away_id: int) -> dict[str, Any]:
    data = raw_fixture.get("data") if isinstance(raw_fixture, dict) else None
    expected_entries = _extract_expected_entries(data)
    home_entries = [entry for entry in expected_entries if str(entry.get("participant_id")) == str(home_id)]
    away_entries = [entry for entry in expected_entries if str(entry.get("participant_id")) == str(away_id)]
    available_type_ids = sorted({
        int(entry["type_id"])
    ...
    return {
        ...
        "home": _build_expected_side_payload(home_entries, participant_id=home_id, location="home"),
        "away": _build_expected_side_payload(away_entries, participant_id=away_id, location="away"),
        "available_type_ids": available_type_ids,
    }


def _extract_lineup_expected(raw_fixture: Any, home_id: int, away_id: int) -> dict[str, list[dict]]:
    data = raw_fixture.get("data") if isinstance(raw_fixture, dict) else None
    ...
    for item in lineups:
        ...
        result[side].append({
            ...
            "formation_field": item.get("formation_field"),
            "expected": [entry for entry in expected if isinstance(entry, dict)],
        })

    return result


def _build_xg_payload(
    raw_fixture: Any,
    home_history_raw: Any,
    away_history_raw: Any,
    home_id: int,
    away_id: int,
) -> Optional[dict]:
    fixture_expected = _extract_fixture_expected(raw_fixture, home_id, away_id)
    lineup_expected = _extract_lineup_expected(raw_fixture, home_id, away_id)
    home_history = _extract_team_expected_history(home_history_raw, participant_id=home_id, location="home")
    away_history = _extract_team_expected_history(away_history_raw, participant_id=away_id, location="away")

    summary_source = None
    home_summary = _summary_from_expected_payload(fixture_expected.get("home"))
    away_summary = _summary_from_expected_payload(fixture_expected.get("away"))
    if home_summary or away_summary:
        summary_source = "fixture_expected"
    else:
        home_summary = _summary_from_expected_payload(home_history)
        away_summary = _summary_from_expected_payload(away_history)
        if home_summary or away_summary:
            summary_source = "team_expected_history"

    all_type_ids = sorted({
        *fixture_expected.get("available_type_ids", []),
        *_type_ids_from_payload(home_history),
        *_type_ids_from_payload(away_history),
        *_type_ids_from_lineups(lineup_expected),
    })

    if not any((
        fixture_expected.get("entries"),
        lineup_expected.get("home"),
        lineup_expected.get("away"),
        home_history,
        away_history,
    )):
        return None
    ...
    return {
        "home_xg_for": (home_summary or {}).get("xg_for"),
        "home_xg_against": (home_summary or {}).get("xg_against"),
        "away_xg_for": (away_summary or {}).get("xg_for"),
        "away_xg_against": (away_summary or {}).get("xg_against"),
        "summary_source": summary_source,
        "available_type_ids": all_type_ids,
...
def _extract_expected_entries(raw: Any) -> list[dict]:
    if raw is None or isinstance(raw, Exception):
        return []
    if isinstance(raw, dict):
        data = raw.get("data", raw)
        if isinstance(data, dict):
            entries = data.get("expected") or data.get("data") or []
        else:
            entries = data
...
def _aggregate_expected_metrics(entries: list[dict]) -> dict[str, float]:
    grouped: dict[int, list[float]] = {}
    for entry in entries:
        type_id = entry.get("type_id")
        value = _expected_entry_value(entry)
        try:
    ...
    for type_id, values in grouped.items():
        if not values:
            continue
        metrics[str(type_id)] = sum(values) / len(values)
        label = _EXPECTED_TYPE_LABELS.get(type_id)
        if label:
            metrics[label] = metrics[str(type_id)]
    return metrics


def _expected_entry_value(entry: dict) -> Optional[float]:
    nested_data = entry.get("data") if isinstance(entry.get("data"), dict) else {}
    raw_value = nested_data.get("value") if nested_data else entry.get("value")
    try:
...
def _summary_from_expected_payload(payload: Optional[dict]) -> Optional[dict]:
    if not payload:
        return None
    metrics = payload.get("metrics") or {}
    xg_for = metrics.get("xg") if metrics.get("xg") is not None else metrics.get(str(_EXPECTED_GOALS_TYPE_ID))
    xg_against = metrics.get("xga") if metrics.get("xga") is not None else metrics.get(str(_EXPECTED_GOALS_AGAINST_TYPE_ID))
...
def _type_ids_from_payload(payload: Optional[dict]) -> set[int]:
    if not payload:
        return set()
    ...
    for entry in entries:
        try:
            result.add(int(entry.get("type_id")))
        except (TypeError, ValueError):
            continue
    return result


def _type_ids_from_lineups(lineup_expected: dict[str, list[dict]]) -> set[int]:
    result = set()
    for items in lineup_expected.values():
        for lineup in items:
            for entry in lineup.get("expected") or []:
                try:
                    result.add(int(entry.get("type_id")))
                except (TypeError, ValueError):
                    continue
...
def _build_xg_summary(home_payload: Optional[dict], away_payload: Optional[dict]) -> Optional[dict[str, float]]:
    ...
    home_xg = home_metrics.get(str(_EXPECTED_GOALS_TYPE_ID))
    away_xg = away_metrics.get(str(_EXPECTED_GOALS_TYPE_ID))
    home_xga = home_metrics.get(str(_EXPECTED_GOALS_AGAINST_TYPE_ID))
    away_xga = away_metrics.get(str(_EXPECTED_GOALS_AGAINST_TYPE_ID))
