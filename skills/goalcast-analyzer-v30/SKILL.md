...
- 置信度 → `goalcast_calculate_confidence(method="v3.0", ...)`
...
**注意**：xG 数据源的选择由 `goalcast_footystats_resolve_match`（DataFusion 层）负责，Skill 不直接调用任何 provider 工具。
...
调用 `goalcast_calculate_confidence(method="v3.0", ...)`：
```
goalcast_calculate_confidence(
    ...
    market_downgraded=True  # v3.0 固定降级
)
```
