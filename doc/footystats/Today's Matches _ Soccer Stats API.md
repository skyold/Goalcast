...
`GET` `https://api.football-data-api.com/todays-matches?key=YOURKEY` Sample Response (Access the URL below)
[https://api.football-data-api.com/todays-matches?key=example](https://api.football-data-api.com/todays-matches?key=example) You must choose the leagues in your settings for the matches to show up here.Returns a maximum of 200 matches per page. Pagination is enabled by default. Add &page=2 to begin paginating.
#### Query Parameters
