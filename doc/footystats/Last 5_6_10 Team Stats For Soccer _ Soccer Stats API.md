...
`GET` `https://api.football-data-api.com/lastx?key=YOURKEY&team_id=*` Sample Response (Access the URL below)
[https://api.football-data-api.com/lastx?key=example&team_id=59](https://api.football-data-api.com/lastx?key=example&team_id=59) 1 query allows you to get all 3 types of stats (last 5 / 6 /10). We are adding last 15 and last 20 stats down the road.Stats(attributes and properties) are identical to the stats from team endpoint.
#### Query Parameters
