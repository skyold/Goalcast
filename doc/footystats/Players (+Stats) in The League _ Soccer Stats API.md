...
`GET` `https://api.football-data-api.com/league-players?key=YOURKEY&season_id=*` Sample Response (Access the URL below)
[https://api.football-data-api.com/league-players?key=example&season_id=2012&include=stats](https://api.football-data-api.com/league-players?key=example&season_id=2012&include=stats) This endpoint provides an array of players that have played in the particular league/season ID. Each player object has attributes described in https://footystats.org/api/documentations/player-individual
#### Query Parameters
