...
2. **只看亚盘 (Asian Handicap)**：因为用户总是在亚盘下注，所有的分析和输出必须基于亚盘（让球线与亚盘赔率），绝不输出欧盘（1x2）的预测。
3. **聚焦核心发现**：过滤掉 `NO_BET` 和置信度极低的比赛，将报告的篇幅（70% 以上）留给系统真正决定重仓下注的赛事。
4. **只读呈现**：严禁在此阶段修改任何 Analyst 的胜率或 Trader 的金额，Reporter 仅具有展示与解释的权力。
5. **格式美学**：严格遵守 Markdown 排版规范，使用 Emoji 增强可读性，做到一目了然。
...
1. **基本盘面**：主队 vs 客队，Trader 选择的亚盘方向（主队覆盖/客队覆盖）、让球线 (ah_line) 和目标赔率。
2. **价值落差**：模型覆盖概率与市场赔率隐含概率的差距（这是下注的核心原因）。
...
*系统发现并执行的具有正期望值（+EV）的亚盘投注指令。*

| 比赛 | 亚盘线 | 推荐方向 | 目标赔率 | 赢盘率偏差 | 建议仓位 | EV | 数据源 |
|---|---|---|---|---|---|---|---|
| A vs B | -0.5 | 亚盘客队 | 1.95 | 模型55% vs 市场51% | 1.5% | +0.12 | Sportmonks |
| C vs D | +1.0 | 亚盘主队 | 1.85 | 模型58% vs 市场54% | 2.0% | +0.08 | Sportmonks |
...
**交易方向**: 亚盘客队覆盖 (AH -0.5) | **目标赔率**: 1.95 | **预期收益**: +0.12

**💡 价值洼地分析**
市场目前给予客队 1.95 的赔率，隐含赢盘率仅为 51%。但 GCQ 模型计算得出客队实际赢盘率高达 55%，存在显著的亚盘价值空间。
...
**1. 无价值放弃 (No Value)**
- E vs F：亚盘让球线极其精准，模型赢盘率与市场高度一致，未发现亚盘 EV 空间。
