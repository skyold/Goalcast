...
class SportmonksCollector:
    ...
    async def collect_match_layers(self, fixture: dict) -> dict[str, Any]:
        ...
        fixture_with_lineups, standings_raw, odds_raw, predictions_raw, xg_home_raw, xg_away_raw, h2h_raw = await asyncio.gather(
            self.provider.get_fixture_by_id(fixture_id, include="lineups"),
        ...
        xg_home = _extract_team_xg_avg(xg_home_raw, participant_id=home_id)
        xg_away = _extract_team_xg_avg(xg_away_raw, participant_id=away_id)
        xg = None
        if xg_home or xg_away:
            xg = {
                "home_xg_for": (xg_home or {}).get("xg_for"),
                "home_xg_against": (xg_home or {}).get("xg_against"),
                "away_xg_for": (xg_away or {}).get("xg_for"),
                "away_xg_against": (xg_away or {}).get("xg_against"),
            }
